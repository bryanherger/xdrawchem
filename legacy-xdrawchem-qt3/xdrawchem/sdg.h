...
class SDG {
 public:
  ...
  arerings = true; }
  void setAtoms(QPtrVector<Atom> a1) { atomSet = a1; }
  void setRings(setOfRings s1) { sssr = s1; }
  QPtrVector<Atom> getAtoms() { return atomSet; }
  void setBL(double bl) { bondLength = bl; }

  QPtrVector<Atom> atomSet;
  setOfRings sssr;
  double bondLength;
  double bondLineSep; 
  double roundFactor;
  ...
  void draw(){
    ...
    QMemArray<int> ilist(10);
    int counter;
    SPoint tempPoint;
    bool vmit;
    QMemArray<double> rot(10);
    int nrOfEdges = 0;
  ...
  int whatToDraw(){
	...
	cout << "whatToDraw(): " << f << endl;
	return f;
      }
    }	
    cout << "whatToDraw(): -1" << endl;
    return -1;
  }
  ...
  void positionRingSubstituents(int ringIndex){
    ...
    QPtrList<Integer> complexAtoms, vect1, vect2, extComplAtoms;
    QMemArray<int> ilist(10);
	...
	do{ 	
	  ...
	  for (int f = 0; f < extComplAtoms.count(); f++){
		...
		cout << "secondRing:" << secondRing->getSortedString() << endl;
		...
		if (complexAtoms.count() == 1 && ringType ==3){
		  ...
		  */	
		  ...
		  if (alph != (double) 0.0)
		    //cout << "Invoking rot-Atom() from line 538" << endl;
		  ...
		  if (stillToPos != 0){
			...
			//cout << "Invoking rot-Atom() from line 719" << endl;
  ...
  int positionAtomSubstituents(int atomIndex, SPoint point, SPoint helpPoint, 
			       QMemArray<int> ilist, int modus, int ir, int igr){
    double ph =  0.0174532925;
	...
	//cout << "Invoking rot-Atom() from line 1079" << endl;
    ...
    //cout << "Invoking rot-Atom() from line 1106" << endl;
  ...
  /* Checks the needs of zatoms neighbors in terms of space and
	   ...
	   */
  QMemArray<double> angleOpt(int zatom, QMemArray<int> rili, bool vmit){
    QMemArray<double> rot(10);
    int    hatom = 0, i, kk, ll, neigh;
    bool   wahl;
    double sum, fewi, phi, alpha;
    QMemArray<int> i1(10); QMemArray<int> i2(10);
    QMemArray<double> i3(10); QMemArray<double> nclass(10);
  ...
  */
  void handleBridgeHeads(int bridgeAtomNumber, double x, double y,
	...
	case 5: 
	  ...
	  /*----------------------------------------------*/
	  //cout << "Invoking rot-Atom() from line 1317" << endl;
	...
	case 6: 
	  ...
	  if ((iz == 2) || (iz == 3)){
	    ...
	    /* if more than one neighbor has to be drawn    */
	    /*----------------------------------------------*/
	    //cout << "Invoking rot-Atom() from line 1335" << endl;
	    tempPoint = rotAtom(SPoint(x, y), SPoint(xx, yy), pp);
	    atomSet[atomSet[bridgeAtomNumber]->intnodeTable[i]]->x = tempPoint.x;
	    atomSet[atomSet[bridgeAtomNumber]->intnodeTable[i]]->y = tempPoint.y;
	  }
  ...
  int resort(QMemArray<double> nclass, int neigh, int zatom, int hatom, 
	     QMemArray<int> rili){
    int    i, j, k, kzaehl, kl;
    double min;

    QMemArray<int> sbinex(10);
    QMemArray<int> sbiord(10);
    QMemArray<int> srili(10);
    QMemArray<double> sclass(10);
    k = 0;   
    for (i = 0; i < neigh; i++){
      if (i != hatom)	{ 
	sclass[k] = nclass[i];
  ...
  */
  ...
  QPtrList<Integer> getIntersection(QMemArray<int> vec1, QMemArray<int> vec2){
    QPtrList<Integer> is;
