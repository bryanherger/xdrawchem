...
class DataStructureTools {
 public:
  static QPtrVector<Atom> cloneAtomSet(QPtrVector<Atom> thisSet)
    {
      QPtrVector<Atom> newAtomSet(thisSet.size());
      int f;
...
class SetOfRingsFinder
{
  ...
  */
  setOfRings findSSSR(QPtrVector<Atom> atSet){
    ringCounter = 0; 
    setOfRings sssr;
    QPtrVector<Atom> tempAtomSet = DataStructureTools::cloneAtomSet(atSet);
    int f;
    ...
    Atom *smallest;
    Ring *ring;
    QPtrVector<Atom> edgeToRemove;
    QPtrVector<Atom> remembernodes;
    ...
    */
    QPtrList<Atom> fullSet;
    QPtrList<Atom> trimSet;
    QPtrList<Atom> nodesN2;  
    ...
    cout << "SSSR:" << sssr.size() << endl;
    for (int h = 0; h < sssr.size(); h++)
      cout << h << ":" << sssr[h]->size() << endl;
    cout << "SSSR ends" << endl;
    return sssr;
  }
  ...
  Ring *findSRing(Atom *rootNode, QPtrVector<Atom> tempAtomSet){
  ...
  QPtrList<Atom> getIntersection(QPtrList<Atom> vec1, QPtrList<Atom> vec2){
    QPtrList<Atom> is;		
    for (int f = 0; f < vec1.count(); f++){
      if (vec2.containsRef(vec1.at(f))) is.append(vec1.at(f));	
    }	
    return is;
  }	
  ...
  QPtrList<Atom> getUnion(QPtrList<Atom> vec1, QPtrList<Atom> vec2){
    QPtrList<Atom> is = vec1;
    for (int f = vec2.count()- 1; f > -1; f--){
      if (!vec1.containsRef(vec2.at(f))) is.append(vec2.at(f));	
    }	
    return is;
  }	
  ...
  QPtrList<Atom> *merge(QPtrList<Atom> *vec1, QPtrList<Atom> *vec2){
    QPtrList<Atom> *result = new QPtrList<Atom>;
    Atom *ta;
    for (ta = vec1->first(); ta != 0; ta = vec1->next())
      result->append(ta);
    for (ta = vec2->first(); ta != 0; ta = vec2->next())
      result->append(ta);
    return result;
  }
  ...
  Ring *prepareRing(QPtrList<Atom> vec, QPtrVector<Atom> tempAtomSet)	{
  ...
  void trim(Atom *n, QPtrVector<Atom> tempAtomSet){
    Atom *conn;
  ...
  void breakBond(Atom *thisNode, QPtrVector<Atom> tempAtomSet){
    int degree = thisNode->degree;
    if (degree == 0) {
      cout << "breakBond: invalid node" << endl;
      return;
    }
  ...
  void breakBond(Atom *from, Atom *to,  QPtrVector<Atom> tempAtomSet){
    int degree, f;
  ...
  /** Restores a bond betwenn nodes 'from' and 'to' of an array of 
	    nodes 'tempAtomSet' */
  void restoreBond(Atom *from, Atom *to,  QPtrVector<Atom> tempAtomSet){
  ...
  QPtrVector<Atom> checkEdges(Ring *ring, QPtrVector<Atom> tempAtomSet){
    QPtrVector<Atom> edge;
    Ring *r1, *r2;
    setOfRings tempSoR;
