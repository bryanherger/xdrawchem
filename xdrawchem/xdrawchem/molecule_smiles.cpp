...
void Molecule::SDG(bool coord) {
  ...
  // get unique points
  up = AllPoints();
  QPtrVector<Atom> atoms(up.count());
...
QString Molecule::ToStringFormat(const char * outFormat) {
  ...
  std::istringstream istream(molfile.latin1());
...
void Molecule::FromSMILES(QString sm) {
  ...
  std::istringstream istream(sm.latin1()); // build a stream on the string
  ...
  */
  ...
  for (thisAtom = myMol.BeginAtom(ait); thisAtom; thisAtom =
	 myMol.NextAtom(ait)) {

    //cout << "Adding OBAtom: " << i++ << " of element#: " <<
    //thisAtom->GetAtomicNum() << " type: " <<
    //  etable.GetSymbol(thisAtom->GetAtomicNum()) << endl;
  ...
  for (thisBond = myMol.BeginBond(bit); thisBond; thisBond =
    ...
    addBond(hashit[thisBond->GetBeginAtom()],
	    hashit[thisBond->GetEndAtom()],
	    1,
	    thisBond->GetBondOrder(),
	    QColor(0,0,0),
	    true);
  }
