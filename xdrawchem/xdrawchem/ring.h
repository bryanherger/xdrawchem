...
class Ring:public QPtrVector<Atom>
{
  ...
  */ 
  ...
  void sort(int nodeNumber){
    cout << nodeNumber << " in " << getSortedString() << endl;
  ...
  void sort2() {
    ...
    for (int h = 0; h < size(); h++) cout << at(h)->number << endl;
  }
  ...
  int getComplexity(QPtrVector<Atom> atomSet){
    int cmpl = 0;
    for (int f = 0; f < size(); f++){
  ...
  bool notAllPositioned(QPtrVector<Atom> atomSet){
    int test;
    for (int f = 0; f < size(); f++){
      //test = (at(f))->number;
  ...
  QMemArray<int> getNodeNumberList(){
    QMemArray<int> arr(1);
    int sz = 0;
    for (int f = 0; f < size(); f++){
      arr.resize(sz+1);
  ...
  QPtrVector<Atom> getEdge(int number) {
    Atom *from, *to;
    if (number < size() - 1){
      from = at(number);
      to =  at(number + 1);
